<div class="space-y-2 lg:space-y-4">
    <x-headline>
        Wedding Registry?
    </x-headline>

    <h4 class="text-2xl">
        A few have enquired about gifts; whether we have a list or similar.
    </h4>

    <p>
        Initially, our answer was no. Then it was a chartiable donation in our names. We have since been
        <span class="italic">convinced</span>
        to setup a Honeymoon Fund.
    </p>

    <p>
        If you feel comfortable doing so, then we welcome you to donate towards our spa retreat.
    </p>

    <flux:button
        variant="primary"
        href="https://settleup.starlingbank.com/alexscottonmaryannebarfield?message=Honeymoon%20Fund"
    >
        Send a gift
    </flux:button>
</div>
